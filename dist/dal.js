...
//delete data
async function dal_delete(id) {
    const response = await tripModel.deleteOne({ _id: id });
    if (!response)
        throw new Err(500, 'the delete been filed');
    return 'delete successfully';
}
//update data
async function dal_update(newData, id) {
    const update = await tripModel.updateOne({ _id: id }, { $set: { ...newData } });
    if (!update)
...
//new trip
const dal_newTrip = async (newData) => {
